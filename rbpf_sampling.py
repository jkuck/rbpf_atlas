...
def associate_measurements_sequentially(particle, meas_source_index, measurement_list, total_target_count, \
    ...
    def get_remaining_meas_count(cur_meas_index, cur_meas_score, measurement_list,\
                                 measurement_scores, meas_source_index):
...
#        score_index = params.get_score_index(meas_score, meas_source_index)
        ...
        cur_birth_prior = 0.0
        for i in range(birth_count+1, min(params.max_birth_count(meas_source_index, meas_score) + 1, remaining_meas_count + birth_count + 1)):
            cur_birth_prior += params.birth_prior(meas_source_index, meas_score, i)*(i - birth_count)/remaining_meas_count 
        proposal_distribution_list.append(cur_birth_prior*params.p_birth_likelihood)
#        for i in range(birth_count+1, min(len(params.birth_probabilities[meas_source_index][score_index]), remaining_meas_count + birth_count + 1)):
#            cur_birth_prior += params.birth_probabilities[meas_source_index][score_index][i]*(i - birth_count)/remaining_meas_count 
#        proposal_distribution_list.append(cur_birth_prior*params.p_birth_likelihood)
        ...
        for i in range(clutter_count+1, min(params.max_clutter_count(meas_source_index, meas_score) + 1, remaining_meas_count + clutter_count + 1)):
            cur_clutter_prior += params.clutter_prior(meas_source_index, meas_score, i)*(i - clutter_count)/remaining_meas_count 
        proposal_distribution_list.append(cur_clutter_prior*params.p_clutter_likelihood)
#        for i in range(clutter_count+1, min(len(params.clutter_probabilities[meas_source_index][score_index]), remaining_meas_count + clutter_count + 1)):
#            cur_clutter_prior += params.clutter_probabilities[meas_source_index][score_index][i]*(i - clutter_count)/remaining_meas_count 
#        proposal_distribution_list.append(cur_clutter_prior*params.p_clutter_likelihood)
        ...
        assert(birth_count <= params.max_birth_count(meas_source_index, meas_score))
